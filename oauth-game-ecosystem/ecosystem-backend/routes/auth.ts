  ...
  new GoogleStrategy(
    ...
    async function (accessToken, refreshToken, profile, cb) {
      ...
      try {
        ...
        let playerOf 
        if(!user){
          playerOf = await openfort.players.createPlayer(profile.emails![0].value as string);
          playerOf = playerOf.body.id
        }else{
...
router.post("/signup", async (req, res) => {
  ...
  if (!email || !password || !game) {
    return res
      .status(400)
      .json({ message: "Email, password and game are required." });
  }
  ...
  const playerOf = await openfort.players.createPlayer(email);

  const user = await db.user.create({
    data: { email, password: hashedPassword, refreshToken: refreshToken, playerOf: playerOf.body.id },
  });
...
router.get(
  ...
  async(req, res) => {
    ...
    try {
      playerInventory=
        await openfort.players.getPlayerInventory(
          player_id,
          chain_id,
        );
    } catch (error) {
